...
if [ $sysOS = "Darwin" ]; then
    ...
    echo "\nexport CLANG_FORMAT_PATH=\"/usr/local/Cellar/clang-format/2019-05-14/share/clang/clang-format.py\"" >> ${shellrc}
    echo "export clang_format_fallback_style=\"$PWD/.clang-format\"" >> ${shellrc}

elif [ $sysOS = "Linux" ]; then
    echo "On Linux"
    echo "export CLANG_FORMAT_PATH to ${shellrc}"
    echo "\nexport CLANG_FORMAT_PATH=\"/usr/share/vim/addons/syntax/clang-format.py\"" >> ${shellrc}
    echo "export clang_format_fallback_style=\"$PWD/.clang-format\"" >> ${shellrc}
else
...
echo "\n# Preferred editor for local and remote sessions" >> ${shellrc}
